...
function addWords(params = {}) {
  ...
  const contenedorEl = document.querySelector(".venta-content");
  ...
  templateEl.content
    .querySelector(".venta-img")
    .setAttribute("src", params.imagen);
  templateEl.content
    .querySelector(".venta-img-dos")
    .setAttribute("src", params.imagenes);

  // Agrego el nombre
  templateEl.content.querySelector(".venta-card-title").textContent =
    params.titulo;

  // Agrego la descripción
  templateEl.content.querySelector(".venta-card-text").textContent =
    params.descripcion;

  // Finalmente, agrego el link...
  //templateEl.content.querySelector(".venta-card-link").href = params.url;
...
function getWords() {
  return fetch(
    "https://cdn.contentful.com/spaces/ip7fn0dyxbho/environments/master/entries?access_token=eapv_7lhVyKtpg4h20F_M2I18cwncppaG1g1hfeSsjo&content_type=ventas"
  )
    .then((resp) => {
      return resp.json();
    })
